...
iotchain-core is at the bottom of the IOTChain stack, many other services can be
 built on top of it.
...
# Integration points: using data generated by iotchain-core

iotchain-core generates several types of data that can be used by applications, depending on use cases.
...
As part of this transition, iotchain-core records:
...
* ICP messages recorded by the validator during the consensus round
...
* it's easy to mix "native IOTChain" data with non native data (such as internal customer information) and keep those consistent
* for more demanding applications, custom views can be built using a pub/sub model where data from iotchain-core is published and consumed by many different services
...
A high resolution custom view allows to build the most sophisticated data model and derive complex data from the IOTChain network.
...
A process (singleton) can import data from the [historical SQL tables](#sql-tables) in iotchain-core database as ledgers close and publish this data to processes that need to maintain a custom view.
...
Right now, iotchain-core uses SQL to store the current state of the ledger.
...
* developers of Horizon have to
  *  keep track of changes in how iotchain-core is storing and updating data in its database
  *  be extremely careful when performing requests as to avoid
     *  inconsistent queries
        *  iotchain-core may change ledger between or while Horizon is making queries
        *  multiple iotchain-core instances may not be in perfect sync with each other
     *  performance problems with poorly designed queries (non-indexed, etc).
* operationally, this creates coupling between iotchain-core and Horizon:
  * a traffic spike in Horizon may cause enough stress on iotchain-core that it loses sync
  * it's hard to "scale out" Horizon

### Additional reading

An overview of how the data flows within iotchain-core is [this data flow presentation](software/core-data-flow.pdf)
...
properly registered by updating its cursor so that iotchain-core can safely
garbage collect data no longer needed.
By default iotchain-core only keeps track of its internal state and will
...
# API to iotchain-core

iotchain-core exposes a simple HTTP based API in particular the `tx` endpoint is used to submit transactions.
