...
the ICP messages for the current ledger (ie: abstain).
...
For that reason, upgrades are only validated during ICP rounds - ie, they are not validated when catching up from history.

As a consequence, there is currently no way for a node to not eventually rejoin the network if it doesn't agree
with the upgrade.

A validator in this situation will disagree with the ICP round with the upgrade (and won't even see the network closing
