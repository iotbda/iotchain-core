---
title: List of metrics exposed by iotchain-core
---

iotchain-core uses libmedida for computing metrics, a detailed description can
...
app.state.current                        | counter   | state (BOOTING=0, JOIN_ICP=1, LEDGER_SYNC=2, CATCHING_UP=3, SYNCED=4, STOPPING=5)
...
icp.envelope.emit                        | meter     | ICP message sent
icp.envelope.invalidsig                  | meter     | envelope failed signature verification
icp.envelope.receive                     | meter     | ICP message received
icp.envelope.sign                        | meter     | envelope signed
icp.envelope.validsig                    | meter     | envelope signature verified
icp.fetch.duration                       | timer     | time to complete fetching
icp.memory.cumulative-statements         | counter   | number of known ICP statements known
icp.nomination.combinecandidates         | meter     | number of candidates per call
icp.pending.discarded                    | counter   | number of discarded envelopes
icp.pending.fetching                     | counter   | number of incomplete envelopes
icp.pending.processed                    | counter   | number of already processed envelopes
icp.pending.ready                        | counter   | number of envelopes ready to process
icp.sync.lost                            | meter     | validator lost sync
icp.timeout.nominate                     | meter     | timeouts in nomination
icp.timeout.prepare                      | meter     | timeouts in ballot protocol
icp.timing.externalized                  | timer     | time spent in ballot protocol
icp.timing.nominated                     | timer     | time spent in nomination
icp.value.invalid                        | meter     | ICP value is invalid
icp.value.valid                          | meter     | ICP value is valid
