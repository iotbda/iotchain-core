...
IOTChain is a decentralized, federated peer-to-peer network that allows people to
send payments in any asset anywhere in the world instantaneously, and with
minimal fee.

`IOTChain-core` is the core component of this network. `IOTChain-core` is a C++
implementation of the IOTChain Consensus Protocol configured to construct a chain
of ledgers that are guaranteed to be in agreement across all the participating
nodes at all times.

For more detail on the IOTChain Consensus Protocol and how it establishes this
guarantee see [`src/icp/readme.md`](/src/icp/readme.md).


- [Building & Installing](/INSTALL.md)
- [IOTChain-core administration](software/admin.md)
- [Architecture](architecture.md)
- [Key Concepts](https://iotbdalliance.com/developers/guides/)
...
* **ICP** is our implementation of the IOTChain Consensus Protocol (ICP). This
  ...
  [`src/icp/readme.md`](/src/icp/readme.md)).

* **Herder** is responsible for interfacing between ICP and the rest of
  `iotchain-core`. Herder provides ICP with concrete implementations of the
  methods ICP uses to communicate with peers, to compare values, to determine
...
* **Overlay** connects to and keeps track of the peers this node knows
  ...
  other data downloads are handled without imposing on the ICP-nodes, see
  [`./architecture.md`](/docs/architecture.md).

* **Ledger** applies the transaction set that is externalized by ICP. It also
...
* **src/crypto** contains standard cryptographic routines, including random
  number generation, hashing, hex encoding and IOTChain Key encoding.
