...
unsigned int
computeDefaultThreshold(ICPQuorumSet const& qset, bool simpleMajority)
{
    unsigned int res = 0;
...
Config::Config() : NODE_SEED(SecretKey::random())
{
    ...
    VERSION_STR = IOTCHAIN_CORE_VERSION;
    ...
    LOG_FILE_PATH = "iotchain-core.%datetime{%Y.%M.%d-%H:%m:%s}.log";
...
void
Config::loadQset(std::shared_ptr<cpptoml::table> group, ICPQuorumSet& qset,
                 int level)
{
    if (!group)
    {
    ...
    std::vector<ValidatorEntry> const& validators)
{
    ...
    if (!LocalNode::isVBlocking(QUORUM_SET, archives))
    {
        ...
        if (!UNSAFE_QUORUM)
        {
            ...
            throw std::invalid_argument("ICP unsafe");
        }
    }
}
...
std::string
Config::toString(ICPQuorumSet const& qset)
{
