...
std::string PersistentState::mapping[kLastEntry] = {
    "lastclosedledger", "historyarchivestate", "forceicponnextlaunch",
    "lasticpdata",      "databaseschema",      "networkpassphrase",
    "ledgerupgrades"};
...
PersistentState::getICPStateAllSlots()
{
    // Collect all slots persisted
    std::vector<std::string> states;
    for (uint32 i = 0; i <= Herder::MAX_SLOTS_TO_REMEMBER; i++)
    {
        auto val = getFromDb(getStoreStateName(kLastICPData, i));
        if (!val.empty())
        {
...
void
PersistentState::setICPStateForSlot(uint64 slot, std::string const& value)
{
    auto slotIdx =
        static_cast<uint32>(slot % (Herder::MAX_SLOTS_TO_REMEMBER + 1));
    updateDb(getStoreStateName(kLastICPData, slotIdx), value);
}
