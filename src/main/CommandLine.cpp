...
CommandLine::ConfigOption::getConfig(bool logToFile) const
{
    Config config;
    auto configFile =
        mConfigFile.empty() ? std::string{"iotchain-core.cfg"} : mConfigFile;
...
int
runForceICP(CommandLineArgs const& args)
{
    CommandLine::ConfigOption configOption;
    auto reset = false;

    auto resetOption = clara::Opt{reset}["--reset"](
        "reset force ICP flag, so next time iotchain-core "
    ...
    return runWithHelp(args, {configurationParser(configOption), resetOption},
                       [&] {
                           setForceICPFlag(configOption.getConfig(), !reset);
                           return 0;
                       });
}
...
{
    std::cout << IOTCHAIN_CORE_VERSION << std::endl;
    return 0;
}
...
handleCommandLine(int argc, char* const* argv)
{
    auto commandLine = CommandLine{
        {{"catchup",
         ...
         {"run", "run iotchain-core node", run},
