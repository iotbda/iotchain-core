...
runWithConfig(Config cfg)
{
    if (cfg.MANUAL_CLOSE)
    {
        ...
        {
            LOG(ERROR) << "Starting iotchain-core in MANUAL_CLOSE mode requires "
                          "NODE_IS_VALIDATOR to be set";
            return 1;
        }

        // in manual close mode, we set FORCE_ICP
        // so that the node starts fully in sync
        // (this is to avoid to force icp all the time when testing)
        cfg.FORCE_ICP = true;
    }

    LOG(INFO) << "Starting iotchain-core " << IOTCHAIN_CORE_VERSION;
...
void
setForceICPFlag(Config cfg, bool set)
{
    VirtualClock clock;
    cfg.setNoListen();
    Application::pointer app = Application::create(clock, cfg, false);

    app->getPersistentState().setState(PersistentState::kForceICPOnNextLaunch,
                                       (set ? "true" : "false"));
    if (set)
    {
        LOG(INFO) << "* ";
        LOG(INFO) << "* The `force icp` flag has been set in the db.";
        LOG(INFO) << "* ";
        LOG(INFO)
            << "* The next launch will start icp from the account balances";
    ...
    {
        LOG(INFO) << "* ";
        LOG(INFO) << "* The `force icp` flag has been cleared.";
