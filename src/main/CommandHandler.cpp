...
CommandHandler::CommandHandler(Application& app) : mApp(app)
{
    ...
    addRoute("icp", &CommandHandler::icpInfo);
...
{
    retStr = "<b>Welcome to iotchain-core!</b><p>";
    retStr +=
        "Supported HTTP commands are listed in the <a href=\""
        "https://github.com/iotbda/iotchain-core/blob/master/docs/software/"
...
CommandHandler::manualClose(std::string const& params, std::string& retStr)
{
    ...
    {
        retStr =
            "Set MANUAL_CLOSE=true in the iotchain-core.cfg if you want this "
            "behavior";
    }
}
...
void
CommandHandler::icpInfo(std::string const& params, std::string& retStr)
{
    std::map<std::string, std::string> retMap;
    http::server::server::parseParams(params, retMap);
    size_t lim = 2;
...
CommandHandler::generateLoad(std::string const& params, std::string& retStr)
{
    ...
    else
    {
        retStr = "Set ARTIFICIALLY_GENERATE_LOAD_FOR_TESTING=true in "
                 "the iotchain-core.cfg if you want this behavior";
    }
}
...
CommandHandler::testAcc(std::string const& params, std::string& retStr)
{
    ...
    else
    {
        ...
        auto acc = iotchain::loadAccount(ltx, key.getPublicKey());
        if (acc)
        {
