...
{
    std::regex rx(
        ".*(ledger|bucket|transactions|results|icp)-[[:xdigit:]]+\\.xdr");
    std::smatch sm;
...
signtxn(std::string const& filename, std::string netId, bool base64)
{
    ...
    {
        if (netId.empty())
            netId = getenv("IOTCHAIN_NETWORK_ID");
        if (netId.empty())
            throw std::runtime_error("missing --netid argument or "
                                     "IOTCHAIN_NETWORK_ID environment variable");
