...
class Config : public std::enable_shared_from_this<Config>
{
    ...
    void loadQset(std::shared_ptr<cpptoml::table> group, ICPQuorumSet& qset,
                  int level);
  ...
  public:
    ...
    // The default way iotchain-core starts is to load the state from disk and
    // catch up to the network before starting ICP. If you need different
    // behavior you need to use new-db or force-icp which sets the following
    // flags:

    // ICP will start running immediately using the current local state to
    ...
    //  set here iotchain-core won't run.
    ...
    iotchain::ICPQuorumSet QUORUM_SET;
    ...
    std::string toString(ICPQuorumSet const& qset);
};
}
