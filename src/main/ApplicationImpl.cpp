...
#define IOTCHAIN_CORE_REAL_TIMER_FOR_CERTAIN_NOT_JUST_VIRTUAL_TIME
...
#include "invariant/ConservationOfIotes.h"
...
ApplicationImpl::initialize(bool createNewDB)
{
    ...
    ConservationOfIotes::registerInvariant(*this);
...
ApplicationImpl::getJsonInfo()
{
    ...
    info["build"] = IOTCHAIN_CORE_VERSION;
    ...
    info["ledger"]["closeTime"] = (Json::UInt64)lcl.header.icpValue.closeTime;
...
ApplicationImpl::start()
{
    ...
    if (mPersistentState->getState(PersistentState::kForceICPOnNextLaunch) ==
        "true")
    {
        if (!mConfig.NODE_IS_VALIDATOR)
        {
            LOG(ERROR) << "Starting iotchain-core in FORCE_ICP mode requires "
                          "NODE_IS_VALIDATOR to be set";
            throw std::invalid_argument("NODE_IS_VALIDATOR not set");
        }
        mConfig.FORCE_ICP = true;
    }
    ...
    mLedgerManager->loadLastKnownLedger(
        [this, &done](asio::error_code const& ec) {
            ...
            if (mConfig.FORCE_ICP)
            {
                std::string flagClearedMsg = "";
                if (mPersistentState->getState(
                        PersistentState::kForceICPOnNextLaunch) == "true")
                {
                    flagClearedMsg = " (`force icp` flag cleared in the db)";
                    mPersistentState->setState(
                        PersistentState::kForceICPOnNextLaunch, "false");
                }

                LOG(INFO) << "* ";
                LOG(INFO) << "* Force-starting icp from the current db state."
                          << flagClearedMsg;
                LOG(INFO) << "* ";
