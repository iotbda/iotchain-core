...
ConservationOfIotes::ConservationOfIotes() : Invariant(false)
{
}

std::shared_ptr<Invariant>
ConservationOfIotes::registerInvariant(Application& app)
{
    return app.getInvariantManager().registerInvariant<ConservationOfIotes>();
}

std::string
ConservationOfIotes::getName() const
{
    return "ConservationOfIotes";
}

std::string
ConservationOfIotes::checkOnOperationApply(Operation const& operation,
                                            ...
                                            LedgerTxnDelta const& ltxDelta)
{
    ...
    int64_t deltaBalances = std::accumulate(
        ...
        [](int64_t lhs, decltype(ltxDelta.entry)::value_type const& rhs) {
            return lhs + iotchain::calculateDeltaBalance(rhs.second.current,
                                                        rhs.second.previous);
        });
