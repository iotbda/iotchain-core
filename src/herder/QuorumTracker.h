...
{
  public:
    using QuorumMap = std::unordered_map<NodeID, ICPQuorumSetPtr>;
  ...
  public:
    ...
    bool expand(NodeID const& id, ICPQuorumSetPtr qSet);

    // rebuild the transitive quorum given a lookup function
    void rebuild(std::function<ICPQuorumSetPtr(NodeID const&)> lookup);
