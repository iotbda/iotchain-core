...
TEST_CASE("quorum tracker", "[quorum][herder][acceptance]")
{
    ...
    // allow ICP messages from other slots to be processed
    herder->getHerderICPDriver().lostSync();
    ...
    auto recvEnvelope = [&](ICPEnvelope envelope, uint64 slotID,
                            SecretKey const& k, ICPQuorumSet const& qSet,
        ...
        envelope.signature = k.sign(xdr::xdr_to_opaque(
            app->getNetworkID(), ENVELOPE_TYPE_ICP, envelope.statement));
        herder->recvICPEnvelope(envelope);
        herder->recvICPQuorumSet(qSetH, qSet);
        for (auto& p : pp)
        {
    ...
    auto recvExternalize = [&](uint64 slotID, SecretKey const& k,
                               ICPQuorumSet const& qSet, ValuesTxSet const& v) {
        ICPEnvelope envelope;
        envelope.statement.pledges.type(ICP_ST_EXTERNALIZE);
    ...
    auto makeValue = [&](int i) {
        ...
        auto sv = IOTChainValue{txSet->getContentsHash(),
                               lcl.header.icpValue.closeTime + i,
                               emptyUpgradeSteps, IOTCHAIN_VALUE_BASIC};
        auto v = xdr::xdr_to_opaque(sv);
        herder->signIOTChainValue(valSigner, sv);
