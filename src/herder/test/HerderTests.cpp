...
makeMultiPayment(iotchain::TestAccount& destAccount, iotchain::TestAccount& src,
                 int nbOps, int64 paymentBase, uint32 extraFee, uint32 feeMult)
{
    std::vector<iotchain::Operation> ops;
...
static void
testICPDriver(uint32 protocolVersion, uint32_t maxTxSize, size_t expectedOps,
              bool checkHighFee, bool withICPsignature)
{
    Config cfg(getTestConfig());
    cfg.LEDGER_PROTOCOL_VERSION = protocolVersion;
    cfg.TESTING_UPGRADE_MAX_TX_SET_SIZE = maxTxSize;
    ...
    auto makeTxPair = [&](HerderImpl& herder, TxSetFramePtr txSet,
        ...
        txSet->sortForHash();
        auto sv = IOTChainValue(txSet->getContentsHash(), closeTime,
                               emptyUpgradeSteps, IOTCHAIN_VALUE_BASIC);
        if (sig)
        {
            herder.signIOTChainValue(root.getSecretKey(), sv);
        }
    ...
    SECTION("combineCandidates")
    {
        ...
        auto addToCandidates = [&](TxPair const& p) {
            ...
            REQUIRE(herder.recvICPEnvelope(envelope) ==
                    Herder::ENVELOPE_STATUS_FETCHING);
            REQUIRE(herder.recvTxSet(p.second->getContentsHash(), *p.second));
        };

        TxSetFramePtr txSet0 = makeTransactions(lcl.hash, 0, 1, 100);
        addToCandidates(makeTxPair(herder, txSet0, 10, withICPsignature));

        Value v;
        IOTChainValue sv;

        v = herder.getHerderICPDriver().combineCandidates(1, candidates);
        xdr::xdr_from_opaque(v, sv);
        REQUIRE(sv.ext.v() == IOTCHAIN_VALUE_BASIC);
        ...
        addToCandidates(makeTxPair(herder, txSet1, 5, withICPsignature));
        v = herder.getHerderICPDriver().combineCandidates(1, candidates);
        xdr::xdr_from_opaque(v, sv);
        REQUIRE(sv.ext.v() == IOTCHAIN_VALUE_BASIC);
        REQUIRE(sv.closeTime == 10);
        ...
        addToCandidates(makeTxPair(herder, txSet2, 20, withICPsignature));
        ...
        if (checkHighFee)
        {
            ...
            addToCandidates(makeTxPair(herder, txSetL, 20, withICPsignature));
            TxSetFramePtr txSetL2 =
                makeTransactions(lcl.hash, maxTxSize, 1, 1000);
            addToCandidates(makeTxPair(herder, txSetL2, 20, withICPsignature));
            v = herder.getHerderICPDriver().combineCandidates(1, candidates);
            xdr::xdr_from_opaque(v, sv);
            REQUIRE(sv.ext.v() == IOTCHAIN_VALUE_BASIC);
    ...
    {
        auto& herder = static_cast<HerderImpl&>(app->getHerder());
        auto& icp = herder.getHerderICPDriver();
        ...
        TxSetFramePtr txSet0 = makeTransactions(lcl.hash, 0, 1, 100);
        {
            ...
            auto p = makeTxPair(herder, txSet0, ct, withICPsignature);
        ...
        SECTION("valid")
        {
            auto nomV = makeTxPair(herder, txSet0, ct, withICPsignature);
            REQUIRE(icp.validateValue(seq, nomV.first, true) ==
                    ICPDriver::kFullyValidatedValue);

            auto balV = makeTxPair(herder, txSet0, ct, false);
            REQUIRE(icp.validateValue(seq, balV.first, false) ==
                    ICPDriver::kFullyValidatedValue);
        }
        SECTION("invalid")
        {
            // nomination, requires signature iff withICPsignature is true
            auto nomV = makeTxPair(herder, txSet0, ct, !withICPsignature);
            REQUIRE(icp.validateValue(seq, nomV.first, true) ==
                    ICPDriver::kInvalidValue);
            ...
            if (withICPsignature)
            {
                ...
                auto checkInvalid = [&](IOTChainValue const& sv) {
                    auto v = xdr::xdr_to_opaque(sv);
                    REQUIRE(icp.validateValue(seq, v, true) ==
                            ICPDriver::kInvalidValue);
                };
    ...
    SECTION("accept qset and txset")
    {
        ...
        auto p1 = makeTxPair(herder, transactions1, 10, withICPsignature);
        auto p2 = makeTxPair(herder, transactions1, 10, withICPsignature);
        ...
        {
            REQUIRE(herder.recvICPEnvelope(saneEnvelopeQ1T1) ==
                    Herder::ENVELOPE_STATUS_FETCHING);
            REQUIRE(herder.recvICPEnvelope(saneEnvelopeQ1T1) ==
                    Herder::ENVELOPE_STATUS_FETCHING);
            REQUIRE(herder.recvICPQuorumSet(saneQSet1Hash, saneQSet1));
            REQUIRE(herder.recvTxSet(p1.second->getContentsHash(), *p1.second));
            // will not return ENVELOPE_STATUS_READY as the recvICPEnvelope() is
            // called internally
        ...
        SECTION("only accepts qset once")
        {
            ...
            {
                REQUIRE(herder.recvICPEnvelope(saneEnvelopeQ1T2) ==
                        Herder::ENVELOPE_STATUS_FETCHING);
                REQUIRE(!herder.recvICPQuorumSet(saneQSet1Hash, saneQSet1));
            }
        }
        ...
        SECTION("only accepts txset once")
        {
            ...
            {
                REQUIRE(herder.recvICPEnvelope(saneEnvelopeQ2T1) ==
                        Herder::ENVELOPE_STATUS_FETCHING);
                REQUIRE(!herder.recvTxSet(p1.second->getContentsHash(),
                                          *p1.second));
            }
        ...
        {
            REQUIRE(!herder.recvICPQuorumSet(saneQSet1Hash, saneQSet1));
            REQUIRE(!herder.recvICPQuorumSet(saneQSet2Hash, saneQSet2));
            REQUIRE(!herder.recvICPQuorumSet(bigQSetHash, bigQSet));
        }
        ...
        {
            REQUIRE(herder.recvICPEnvelope(bigEnvelope) ==
                    Herder::ENVELOPE_STATUS_FETCHING);
            REQUIRE(!herder.recvICPQuorumSet(bigQSetHash, bigQSet));
        }
...
TEST_CASE("ICP Driver", "[herder][acceptance]")
{
    SECTION("protocol 10")
    {
        testICPDriver(10, 10, 10, false, false);
    }
    SECTION("protocol current")
    {
        testICPDriver(Config::CURRENT_LEDGER_PROTOCOL_VERSION, 1000, 15, true,
                      true);
    }
}

TEST_CASE("ICP State", "[herder][acceptance]")
{
    ...
    // but for "Force ICP" test there are 3 nodes and only 2 have previous
    ...
    auto doTest = [&](bool forceICP) {
        ...
        // crank a bit (nothing should happen, node 2 is waiting for ICP
        // messages)
        ...
        // nodes 0 and 1 have lost their ICP state as they got restarted
...
TEST_CASE("In quorum filtering", "[quorum][herder][acceptance]")
{
    ...
    // process icp messages for each core node
    auto checkCoreNodes =
        [&](std::function<void(std::vector<ICPEnvelope> const&)> proc) {
            for (auto const& k : qSetBase.validators)
            {
                ...
                auto state =
                    herder.getICP().getCurrentState(lcl.header.ledgerSeq);
                proc(state);
            }
        };

    // none of the messages from the extra nodes should be present
    checkCoreNodes([&](std::vector<ICPEnvelope> const& envs) {
