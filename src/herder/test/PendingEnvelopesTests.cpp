...
TEST_CASE("PendingEnvelopes recvICPEnvelope", "[herder]")
{
    ...
    SECTION("return FETCHING when first receiving envelope")
    {
        ...
        // call
        REQUIRE(pendingEnvelopes.recvICPEnvelope(saneEnvelope) ==
        ...
        {
            REQUIRE(pendingEnvelopes.recvICPQuorumSet(saneQSetHash, saneQSet));
            REQUIRE(pendingEnvelopes.recvICPEnvelope(saneEnvelope) ==
                    Herder::ENVELOPE_STATUS_FETCHING);
            REQUIRE(!pendingEnvelopes.recvICPQuorumSet(saneQSetHash, saneQSet));
            REQUIRE(pendingEnvelopes.recvICPEnvelope(saneEnvelope) ==
            ...
            "that were manually added before")
    {
        ...
        {
            pendingEnvelopes.addICPQuorumSet(saneQSetHash, saneQSet);
    ...
    {
        REQUIRE(pendingEnvelopes.recvICPEnvelope(bigEnvelope) ==
                Herder::ENVELOPE_STATUS_FETCHING);

        SECTION("quorum set first")
        {
            REQUIRE(!pendingEnvelopes.recvICPQuorumSet(bigQSetHash, bigQSet));
            REQUIRE(!pendingEnvelopes.recvTxSet(p.second->getContentsHash(),
                                                p.second));
            REQUIRE(pendingEnvelopes.recvICPEnvelope(bigEnvelope) ==
            ...
            "tx set")
    {
                ...
                "MAX_SLOTS_TO_REMEMBER")
        {
            ...
            REQUIRE(pendingEnvelopes.recvICPEnvelope(saneEnvelope2) ==
                    Herder::ENVELOPE_STATUS_READY);
            pendingEnvelopes.eraseBelow(saneEnvelope3.statement.slotIndex -
                                        Herder::MAX_SLOTS_TO_REMEMBER);
            REQUIRE(pendingEnvelopes.recvICPEnvelope(saneEnvelope3) ==
                    Herder::ENVELOPE_STATUS_READY);
        }
