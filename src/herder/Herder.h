 ...
 *
 * Drives the ICP consensus protocol, is responsible for collecting Txs and
...
class Herder
{
  public:
    ...
    // Maximum timeout for ICP consensus.
    static std::chrono::seconds const MAX_ICP_TIMEOUT_SECONDS;
    ...
    enum EnvelopeStatus
    {
        ...
        // current call to recvICPEnvelope() was the first when the envelope
    ...
    virtual bool recvICPQuorumSet(Hash const& hash,
                                  ICPQuorumSet const& qset) = 0;
    ...
    virtual void peerDoesntHave(iotchain::MessageType type,
    ...
    virtual EnvelopeStatus recvICPEnvelope(ICPEnvelope const& envelope) = 0;

    // We are learning about a new fully-fetched envelope.
    virtual EnvelopeStatus recvICPEnvelope(ICPEnvelope const& envelope,
                                           const ICPQuorumSet& qset,
                                           TxSetFrame txset) = 0;

    // a peer needs our ICP state
    virtual void sendICPStateToPeer(uint32 ledgerSeq, Peer::pointer peer) = 0;
    ...
    // lookup a nodeID in config and in ICP messages
