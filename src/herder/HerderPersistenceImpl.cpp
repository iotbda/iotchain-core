...
void
HerderPersistenceImpl::saveICPHistory(uint32_t seq,
                                      ...
                                      QuorumTracker::QuorumMap const& qmap)
{
    ...
    auto usedQSets = std::unordered_map<Hash, ICPQuorumSetPtr>{};
...

    {
        auto prepClean = db.getPreparedStatement(
            "DELETE FROM icphistory WHERE ledgerseq =:l");

        auto& st = prepClean.statement();
        st.exchange(soci::use(seq));
        st.define_and_bind();
        {
            auto timer = db.getDeleteTimer("icphistory");
            st.execute(true);
        }
    }
    for (auto const& e : envs)
    {
        ...
        auto prepEnv =
            db.getPreparedStatement("INSERT INTO icphistory "
        ...
        st.define_and_bind();
        {
            auto timer = db.getInsertTimer("icphistory");
            st.execute(true);
        }
    ...
    for (auto const& p : usedQSets)
    {
        ...
        auto prepUpQSet =
            db.getPreparedStatement("UPDATE icpquorums SET "
        ...
        {
            auto timer = db.getInsertTimer("icpquorums");
            stUp.execute(true);
        }
...
size_t
HerderPersistence::copyICPHistoryToStream(Database& db, soci::session& sess,
                                          uint32_t ledgerSeq,
                                          ...
                                          XDROutputFileStream& icpHistory)
{
    ...
    for (uint32_t curLedgerSeq = begin; curLedgerSeq < end; curLedgerSeq++)
    {
        ...
        // fetch ICP messages from history
        {
            std::string envB64;

            auto timer = db.getSelectTimer("icphistory");

            soci::statement st =
                (sess.prepare << "SELECT envelope FROM icphistory "
                                ...
                                Hash const& qSetHash)
{
    ...
    auto timer = db.getSelectTimer("icpquorums");

    soci::statement st = (sess.prepare << "SELECT qset FROM icpquorums "
                                          "WHERE qsethash = :h",
    ...
    if (st.got_data())
    {
        ...
        res = std::make_shared<ICPQuorumSet>(std::move(qset));
    }
    return res;
}
...
{
    db.getSession() << "DROP TABLE IF EXISTS icphistory";

    db.getSession() << "DROP TABLE IF EXISTS icpquorums";

    db.getSession() << "CREATE TABLE icphistory ("
                       "nodeid      CHARACTER(56) NOT NULL,"
                       "ledgerseq   INT NOT NULL CHECK (ledgerseq >= 0),"
                       "envelope    TEXT NOT NULL"
                       ")";

    db.getSession() << "CREATE INDEX icpenvsbyseq ON icphistory(ledgerseq)";

    db.getSession() << "CREATE TABLE icpquorums ("
    ...
    db.getSession()
        << "CREATE INDEX icpquorumsbyseq ON icpquorums(lastledgerseq)";
                                    ...
                                    uint32_t count)
{
    DatabaseUtils::deleteOldEntriesHelper(db.getSession(), ledgerSeq, count,
                                          "icphistory", "ledgerseq");
    DatabaseUtils::deleteOldEntriesHelper(db.getSession(), ledgerSeq, count,
                                          "icpquorums", "lastledgerseq");
}
}
