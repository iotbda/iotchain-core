...
// elsewhere in iotchain-core so there's a little work up front converting
// representations.
//
...
// A QBitSet is the "fast" representation of a ICPQuorumSet. It includes both a
...
class QuorumIntersectionCheckerImpl : public iotchain::QuorumIntersectionChecker
{

    iotchain::Config const& mCfg;
    ...
    // reporting.
    mutable std::pair<std::vector<iotchain::PublicKey>,
                      std::vector<iotchain::PublicKey>>
        mPotentialSplit;
    ...
    std::vector<iotchain::PublicKey> mBitNumPubKeys;
    std::unordered_map<iotchain::PublicKey, size_t> mPubKeyBitNums;
    QGraph mGraph;
    ...
    QBitSet convertICPQuorumSet(iotchain::ICPQuorumSet const& sqs);
    void buildGraph(iotchain::QuorumTracker::QuorumMap const& qmap);
    void buildSCCs();
  ...
  public:
    QuorumIntersectionCheckerImpl(iotchain::QuorumTracker::QuorumMap const& qmap,
                                  iotchain::Config const& cfg,
                                  bool quiet = false);
    bool networkEnjoysQuorumIntersection() const override;

    std::pair<std::vector<iotchain::PublicKey>, std::vector<iotchain::PublicKey>>
