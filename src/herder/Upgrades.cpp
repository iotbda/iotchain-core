...
void
save(Archive& ar, iotchain::Upgrades::UpgradeParameters const& p)
{
    ar(make_nvp("time", iotchain::VirtualClock::to_time_t(p.mUpgradeTime)));
...
template <class Archive>
void
load(Archive& ar, iotchain::Upgrades::UpgradeParameters& o)
{
    time_t t;
    ar(make_nvp("time", t));
    o.mUpgradeTime = iotchain::VirtualClock::from_time_t(t);
...
Upgrades::createUpgradesFor(LedgerHeader const& header) const
{
    ...
    if (!timeForUpgrade(header.icpValue.closeTime))
    {
        return result;
    }
               ...
               AccountID const& accountID, Asset const& asset, int64_t delta)
{
    ...
    if (iter->second)
    {
        if (!iotchain::addBalance(*iter->second, delta))
        {
            iter->second.reset();
        }
    }
                                        ...
                                        AbstractLedgerTxn& ltx)
{
    ...
    {
        auto trust = iotchain::loadTrustLineWithoutRecord(ltx, accountID, asset);
...
prepareLiabilities(AbstractLedgerTxn& ltx, LedgerTxnHeader const& header)
{
    ...
    for (auto& accountOffers : offersByAccount)
    {
        ...
        auto accountEntry = iotchain::loadAccount(ltx, accountOffers.first);
        if (!accountEntry)
        {
        ...
        for (auto& offerEntry : accountOffers.second)
        {
            ...
            {
                iotchain::addNumEntries(header, accountEntry, -1);
            }
        ...
        for (auto const& assetLiabilities : liabilities)
        {
            ...
            {
                auto trustEntry =
                    iotchain::loadTrustLine(ltx, accountOffers.first, asset);
                int64_t deltaSelling =
