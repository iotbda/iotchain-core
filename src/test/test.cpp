...
bool force_sqlite = (std::getenv("IOTCHAIN_FORCE_SQLITE") != nullptr);
...
getTestConfig(int instanceNumber, Config::TestDbMode mode)
{
    ...
    {
        gTestRoots.emplace_back("iotchain-core-test");
        ...
        sstream << "iotchain" << instanceNumber << ".log";
        ...
        // We set a secret key by default as FORCE_ICP is true by
     ...
     std::vector<std::string> const& metrics)
{
    ...
    // not mixed with iotchain-core logging.
    Logging::setFmt("<test>");
    ...
    LOG(INFO) << "Testing iotchain-core " << IOTCHAIN_CORE_VERSION;
    ...
    cli |= clara::Opt(gBaseInstance, "offset")["--base-instance"](
        ...
        "iotchain-core can run tests concurrently");
    session.cli(cli);
...
runTest(CommandLineArgs const& args)
{
    ...
    if (seed != 0)
    {
        iotchain::gRandomEngine.seed(seed);
    }
