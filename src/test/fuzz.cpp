 ...
 *     random IOTChainMessages. This is the mode you use to generate seed data
 *     for the external fuzzer's corpus.
 *
 *   - In fuzz mode it reads back a file and appplies it to a pair of of
 *     iotchain-cores in loopback mode, cranking the I/O loop to simulate
...
std::string
msgSummary(IOTChainMessage const& m)
{
...
bool
tryRead(XDRInputFileStream& in, IOTChainMessage& m)
{
    try
    {
        return in.readOne(m);
     ...
     std::vector<std::string> const& metrics)
{
    ...
    LOG(INFO) << "Fuzzing iotchain-core " << IOTCHAIN_CORE_VERSION;
...
genfuzz(std::string const& filename)
{
    ...
    out.open(filename);
    autocheck::generator<IOTChainMessage> gen;
    for (size_t i = 0; i < n; ++i)
    {
        try
        {
            IOTChainMessage m(gen(10));
