...
TestAccount::updateSequenceNumber()
{
    if (mSn == 0)
    {
        ...
        auto entry = iotchain::loadAccount(ltx, getPublicKey());
        if (entry)
        {
...
TestAccount::create(SecretKey const& secretKey, uint64_t initialBalance)
{
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto entry = iotchain::loadAccount(ltx, publicKey);
        if (entry)
        {
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto destAfter = iotchain::loadAccount(ltx, publicKey);
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        REQUIRE(iotchain::loadAccount(ltx, publicKey));
    }
...
TestAccount::merge(PublicKey const& into)
{
    ...
    LedgerTxn ltx(mApp.getLedgerTxnRoot());
    REQUIRE(iotchain::loadAccount(ltx, into));
    REQUIRE(!iotchain::loadAccount(ltx, getPublicKey()));
}
...
TestAccount::manageData(std::string const& name, DataValue* value)
{
    ...
    auto data = iotchain::loadData(ls, getPublicKey(), name);
    if (value)
    {
        REQUIRE(data);
...
TestAccount::pay(PublicKey const& destination, int64_t amount)
{
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto toAccountEntry = iotchain::loadAccount(ltx, destination);
        toAccount =
            toAccountEntry
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto toAccountAfter = iotchain::loadAccount(ltx, destination);
        // check that the target account didn't change
