...
The `get` and `put` commands define subprocesses that `iotchain-core` will
...
called `s3iotchain` based on the [AWS S3](https://aws.amazon.com/s3/) storage system:

~~~~
[HISTORY.s3iotchain]
get="curl http://history.iotchain.dev/{0} -o {1}"
put="aws s3 cp {0} s3://history.iotchain.dev/{1}"
~~~~

In this example, `iotchain-core` will use the `curl` command to fetch history and
