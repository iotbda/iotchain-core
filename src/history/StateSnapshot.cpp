...
StateSnapshot::StateSnapshot(Application& app, HistoryArchiveState const& state)
    ...
    , mICPHistorySnapFile(std::make_shared<FileTransferInfo>(
          mSnapDir, HISTORY_FILE_TYPE_ICP, mLocalState.currentLedger))
...
StateSnapshot::writeHistoryBlocks() const
{
    ...
    size_t nHeaders;
    {
        ...
        icpHistory.open(mICPHistorySnapFile->localPath_nogz());
        ...
        nbICPMessages = HerderPersistence::copyICPHistoryToStream(
            mApp.getDatabase(), sess, begin, count, icpHistory);

        CLOG(DEBUG, "History")
            << "Wrote " << nbICPMessages << " ICP messages to "
            << mICPHistorySnapFile->localPath_nogz();
    }

    if (nbICPMessages == 0)
    {
        // don't upload empty files
        std::remove(mICPHistorySnapFile->localPath_nogz().c_str());
    }
...
StateSnapshot::differingHASFiles(HistoryArchiveState const& other)
{
    ...
    addIfExists(mICPHistorySnapFile);
