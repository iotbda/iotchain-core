...
void
InferredQuorum::noteICPHistory(ICPHistoryEntry const& hist)
{
...
void
InferredQuorum::noteQset(ICPQuorumSet const& qset)
{
...
InferredQuorum::getQuorumMap() const
{
    ...
    for (auto const& pair : mQsetHashes)
    {
        ...
        for (auto i = pair.second.rbegin(); i != pair.second.rend(); ++i)
        {
            ...
            if (qi != mQsets.end())
            {
                ICPQuorumSetPtr p = std::make_shared<ICPQuorumSet>(qi->second);
                qm[pair.first] = p;
                break;
            }
        }
