...
FetchRecentQsetsWork::doReset()
{
    ...
    mDownloadICPMessagesWork.reset();
    mDownloadDir =
...
FetchRecentQsetsWork::doWork()
{
    ...
    // Phase 2: download some ICP messages; for now we just pull the past 100
    ...
    if (!mDownloadICPMessagesWork)
    {
        CLOG(INFO, "History") << "Downloading historical ICP messages: ["
                              << firstSeq << ", " << lastSeq << "]";
        auto range = CheckpointRange{firstSeq, lastSeq, step};
        mDownloadICPMessagesWork = addWork<BatchDownloadWork>(
            range, HISTORY_FILE_TYPE_ICP, *mDownloadDir);
        return State::WORK_RUNNING;
    }
    else if (mDownloadICPMessagesWork->getState() != State::WORK_SUCCESS)
    {
        return mDownloadICPMessagesWork->getState();
    }
    ...
    for (uint32_t i = firstSeq; i <= lastSeq; i += step)
    {
        ...
        FileTransferInfo fi(*mDownloadDir, HISTORY_FILE_TYPE_ICP, i);
        try
        {
        ...
        while (in && in.readOne(tmp))
        {
            ...
            mInferredQuorum.noteICPHistory(tmp);
        }
    }
