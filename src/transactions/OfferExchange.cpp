           ...
           std::vector<ClaimOfferAtom>& offerTrail)
{
    ...
    {
        auto accountB = iotchain::loadAccountWithoutRecord(ltx, accountBID);
        if (!accountB)
        {
    ...
    { // entire offer is taken
        ...
        auto accountB = iotchain::loadAccount(ltx, accountBID);
    ...
    if (numSheepSend != 0)
    {
        ...
        if (sheep.type() == ASSET_TYPE_NATIVE)
        {
            auto accountB = iotchain::loadAccount(ltxInner, accountBID);
        ...
        {
            auto sheepLineAccountB =
                iotchain::loadTrustLine(ltxInner, accountBID, sheep);
    ...
    if (numWheatReceived != 0)
    {
        ...
        {
            auto wheatLineAccountB =
                iotchain::loadTrustLine(ltxInner, accountBID, wheat);
              ...
              bool isPathPayment, std::vector<ClaimOfferAtom>& offerTrail)
{
    ...
    if (!iotchain::loadAccountWithoutRecord(ltx, accountBID))
    {
        throw std::runtime_error(
            "invalid database state: offer must have matching account");
    }
    ...
    {
        accountB = iotchain::loadAccount(ltx, accountBID);
    }
                                   ...
                                   : CrossOfferResult::eOfferPartial;
    {
        ...
        {
            sellingWheatOffer.erase();
            accountB = iotchain::loadAccount(ltxInner, accountBID);
