                              ...
                              AccountID const& accountID)
{
    ...
    {
        // this is buggy caching that existed in old versions of the protocol
        auto res = iotchain::loadAccount(ltx, accountID);
        if (res)
        {
    ...
    {
        return iotchain::loadAccount(ltx, accountID);
    }
}
...
TransactionFrame::isTooEarly(LedgerTxnHeader const& header) const
{
    ...
    {
        uint64 closeTime = header.current().icpValue.closeTime;
...
TransactionFrame::processFeeSeqNum(AbstractLedgerTxn& ltx, int64_t baseFee)
{
    ...
    if (fee > 0)
    {
        ...
        iotchain::addBalance(acc.balance, -fee);
...
{
    auto account = iotchain::loadAccount(ltx, accountID);
    if (!account)
    {
                                      ...
                                      SignerKey const& signerKey) const
{
    ...
    {
        auto removed = iotchain::addNumEntries(header, account, -1);
...
IOTChainMessage
TransactionFrame::toIOTChainMessage() const
{
    IOTChainMessage msg;
