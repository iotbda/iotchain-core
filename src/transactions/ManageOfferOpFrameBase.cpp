...
ManageOfferOpFrameBase::checkOfferValid(AbstractLedgerTxn& ltxOuter)
{
    ...
    if (mSheep.type() != ASSET_TYPE_NATIVE)
    {
        ...
        auto issuer = iotchain::loadAccount(ltx, getIssuer(mSheep));
        if (!issuer)
        {
    ...
    if (mWheat.type() != ASSET_TYPE_NATIVE)
    {
        ...
        auto issuer = iotchain::loadAccount(ltx, getIssuer(mWheat));
        if (!issuer)
        {
...
ManageOfferOpFrameBase::doApply(AbstractLedgerTxn& ltxOuter)
{
    ...
    { // modifying an old offer
        auto sellSheepOffer = iotchain::loadOffer(ltx, getSourceID(), mOfferID);
        if (!sellSheepOffer)
        {
            setResultNotFound();
            return false;
    ...
    if (!isDeleteOffer())
    {
        ...
        if (header.current().ledgerVersion >= 10)
        {
            ...
            {
                auto sourceAccount =
                    iotchain::loadAccountWithoutRecord(ltx, getSourceID());
