                   ...
                   std::function<int(int)> getVote)
{
    ...
    for (int i = 0; i < nbAccounts; i++)
    {
        ...
        if (bal >= 0)
        {
            ...
            auto account = iotchain::loadAccount(ltx, to.getPublicKey());
                  ...
                  std::function<int(int)> getVote, Application& app)
{
    ...
    for (auto w : winners)
    {
        ...
        if (ledgerVersion >= 10)
        {
            ...
            auto winner =
                iotchain::loadAccount(ltx, getTestAccount(w).getPublicKey());
            toDoleToThis =
            ...
            std::function<int(int)> getVote, int expectedWinnerCount)
{
    ...
    for (int i = 0; i < nbAccounts; i++)
    {
        ...
        else
        {
            LedgerTxn ltx(app.getLedgerTxnRoot());
            auto account =
                iotchain::loadAccount(ltx, getTestAccount(i).getPublicKey());
    ...
    for (int i = 0; i < nbAccounts; i++)
    {
        ...
        else
        {
            {
                LedgerTxn ltx(app.getLedgerTxnRoot());
                auto account = iotchain::loadAccount(ltx, k.getPublicKey());
                auto const& ae = account.current().data.account();
