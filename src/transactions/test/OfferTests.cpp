...
TEST_CASE("create offer", "[tx][offers]")
{
    ...
    SECTION("create offer errors")
    {
        ...
        SECTION("create offer without IOT to make for reserve")
        {
            auto a1 = root.create("A", minBalance2);
            a1.changeTrust(idr, trustLineLimit);
            a1.changeTrust(usd, trustLineLimit);
                ...
                for (auto const& p : invalidPrices)
                {
                    REQUIRE_THROWS_AS(
                        market.requireChangesWithOffer(
                            {},
                            [&] {
                                return market.addOffer(a, {iot, idr, p, 150});
                            }),
                        ex_MANAGE_SELL_OFFER_MALFORMED);
                }
            });
        ...
        SECTION("cancel offer with full buying trust line")
        {
            ...
            for_versions_from(10, *app, [&] {
                int64_t usdBuyingLiabilities = 0;
                {
                    ...
                    auto trustLine = iotchain::loadTrustLine(ltx, a1, usd);
    ...
    SECTION("create offer")
    {
        ...
        SECTION("idr -> iot")
        {
            for_all_versions(*app, [&] {
                market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(a1, {iot, idr, Price{3, 2}, 100});
                });
            });
        }
        ...
        SECTION("iot -> idr")
        {
            ...
            SECTION("crossing + create")
            {
                ...
                auto a1Offer = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(a1, {idr, iot, Price{1, 1}, a1IDrs});
                });
                ...
                auto checkCrossed = [&](TestAccount& b1, int64 actualPayment,
                    ...
                    auto b1Offer =
                        market.addOffer(b1, {iot, idr, oneone, offerAmount},
                                        OfferState::DELETED);
                    market.requireBalances(
                        {{a1,
                          {{iot, a1Before + actualPayment},
                           {idr, trustLineBalance - actualPayment}}},
                         {b1,
                          {{iot, b1Before - txfee - actualPayment},
                           {idr, actualPayment}}}});
                };
    ...
    SECTION("crossing offers with rounding")
    {
        ...
        auto bidding = OfferState{iot, idr, bidPrice, bidAmount};
        auto asking = OfferState{idr, iot, askPrice, askAmount};
        ...
        SECTION("bid before ask uses bid price")
        {
            ...
            for_versions_to(2, *app, [&] {
                ...
                // 4715278
                // (rounding down)
                auto updatedAsking = OfferState{idr, iot, askPrice, 4715278};
                // rounding error, should be 0
                auto updatedBidding = OfferState{iot, idr, bidPrice, 1};
            ...
            for_versions(3, 9, *app, [&] {
                ...
                // 4715277
                // (rounding up)
                auto updatedAsking = OfferState{idr, iot, askPrice, 4715277};
                market.requireChangesWithOffer(
        ...
        SECTION("ask before bid uses ask price")
        {
            for_all_versions(*app, [&] {
                ...
                auto updatedBidding = OfferState{iot, idr, bidPrice, 61363625};
                market.requireChangesWithOffer(
    ...
    SECTION("updated offers respect reserve")
    {
        ...
        for_versions_to(9, *app, [&] {
            auto offer = market.requireChangesWithOffer({}, [&] {
                return market.addOffer(a1, {iot, usd, oneone, 110});
            });
            market.requireChangesWithOffer({}, [&] {
                return market.updateOffer(a1, offer.key.offerID,
                                          {iot, usd, oneone, 111},
    ...
    SECTION("max liabilities")
    {
        SECTION("buying non-native")
        {
            for_versions_from(10, *app, [&] {
                ...
                auto market = TestMarket{*app};
                auto offer = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(acc1, {iot, usd, Price{1, 1}, 1000});
                });
                market.requireChangesWithOffer(
                    {{offer.key, OfferState::DELETED}}, [&] {
                        return market.addOffer(acc2,
                                               {usd, iot, Price{1, 1}, 1000},
                                               OfferState::DELETED);
                    });
            });
        }
        SECTION("selling non-native")
        {
            for_versions_from(10, *app, [&] {
                ...
                auto market = TestMarket{*app};
                auto offer = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(acc2, {usd, iot, Price{1, 1}, 1000});
                });
                market.requireChangesWithOffer(
                    {{offer.key, OfferState::DELETED}}, [&] {
                        return market.addOffer(acc1,
                                               {iot, usd, Price{1, 1}, 1000},
                                               OfferState::DELETED);
                    });
            });
        }
    ...
    SECTION("cannot create offer that would lead to excess liabilities")
    {
        SECTION("native selling liabilities")
        {
            for_versions_from(10, *app, [&] {
                ...
                root.pay(acc1, iot, 500 + txfee);
                REQUIRE_THROWS_AS(
                    market.addOffer(acc1, {iot, usd, Price{1, 1}, 501}),
                    ex_MANAGE_SELL_OFFER_UNDERFUNDED);
                root.pay(acc1, iot, txfee);
                market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(acc1, {iot, usd, Price{1, 1}, 500});
                });

                // Test when existing offers
                auto reserve = app->getLedgerManager().getLastReserve();
                root.pay(acc1, iot, 500 + txfee + reserve);
                REQUIRE_THROWS_AS(
                    market.addOffer(acc1, {iot, usd, Price{1, 1}, 501}),
                    ex_MANAGE_SELL_OFFER_UNDERFUNDED);
                root.pay(acc1, iot, txfee);
        ...
        SECTION("native buying liabilities")
        {
            for_versions_from(10, *app, [&] {
                ...
                // Test when no existing offers
                root.pay(acc1, iot, 2 * txfee);
                REQUIRE_THROWS_AS(
                    market.addOffer(acc1, {usd, iot, Price{1, 1},
                ...
                market.requireChangesWithOffer({}, [&] {
                    return market.updateOffer(
                        acc1, o1.key.offerID,
                        {usd, iot, Price{1, 1}, INT64_MAX - minBalance - 500});
                });

                // Test when existing offers
                root.pay(acc1, iot, txfee);
                REQUIRE_THROWS_AS(
                    market.addOffer(acc1, {usd, iot, Price{1, 1}, 501}),
                    ex_MANAGE_SELL_OFFER_LINE_FULL);
                root.pay(acc1, iot, txfee);
                auto o2 = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(acc1, {usd, iot, Price{1, 1}, 500});
                });
            });
        }
    ...
    SECTION("cannot modify offer that would lead to excess liabilities")
    {
        SECTION("native selling liabilities")
        {
            for_versions_from(10, *app, [&] {
                ...
                root.pay(acc1, iot, 500 + txfee);
                auto o1 = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(acc1, {iot, usd, Price{1, 1}, 250});
                });
                root.pay(acc1, iot, txfee);
                REQUIRE_THROWS_AS(
                    market.updateOffer(acc1, o1.key.offerID,
                                       {iot, usd, Price{1, 1}, 501}),
                    ex_MANAGE_SELL_OFFER_UNDERFUNDED);
                root.pay(acc1, iot, txfee);
                market.requireChangesWithOffer({}, [&] {
                    return market.updateOffer(acc1, o1.key.offerID,
                                              {iot, usd, Price{1, 1}, 500});
                });
        ...
        SECTION("native buying liabilities")
        {
            for_versions_from(10, *app, [&] {
                ...
                REQUIRE_THROWS_AS(
                    market.updateOffer(
                        acc1, o1.key.offerID,
                        {usd, iot, Price{1, 1}, INT64_MAX - minBalance + 1}),
                    ex_MANAGE_SELL_OFFER_LINE_FULL);
                root.pay(acc1, iot, txfee);
                market.requireChangesWithOffer({}, [&] {
                    return market.updateOffer(
                        acc1, o1.key.offerID,
                        {usd, iot, Price{1, 1}, INT64_MAX - minBalance});
                });
                ...
                root.pay(acc1, iot, 2 * txfee);
                auto o2 = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(acc1, {usd, iot, Price{1, 1}, 250});
                });
                root.pay(acc1, iot, txfee);
                REQUIRE_THROWS_AS(
                    market.updateOffer(acc1, o2.key.offerID,
                                       {usd, iot, Price{1, 1}, 501}),
                    ex_MANAGE_SELL_OFFER_LINE_FULL);
                root.pay(acc1, iot, txfee);
                market.requireChangesWithOffer({}, [&] {
                    return market.updateOffer(acc1, o2.key.offerID,
                                              {usd, iot, Price{1, 1}, 500});
                });
            });
        }
    ...
    SECTION("cannot create unauthorized offer")
    {
        for_all_versions(*app, [&] {
            ...
            REQUIRE_THROWS_AS(market.addOffer(acc1, {idr, iot, Price{1, 1}, 1}),
                              ex_MANAGE_SELL_OFFER_SELL_NOT_AUTHORIZED);
            REQUIRE_THROWS_AS(market.addOffer(acc1, {iot, idr, Price{1, 1}, 1}),
    ...
    SECTION("modify offer price with liabilities")
    {
        ...
        SECTION("buying native")
        {
            ...
            TestMarket market(*app);
            auto o1 = market.requireChangesWithOffer({}, [&] {
                return market.addOffer(acc1, {idr, iot, Price{1, 1}, 500});
            });
            ...
            for_versions_from(10, *app, [&] {
                ...
                SECTION("decrease price")
                {
                    auto o2 = market.requireChangesWithOffer({}, [&] {
                        return market.addOffer(
                            acc1, {usd, iot, Price{1, 1},
                                   INT64_MAX - minBalance - 500 - txfee});
                    });
                    root.pay(acc1, txfee);
                    market.requireChangesWithOffer({}, [&] {
                        return market.updateOffer(acc1, o1.key.offerID,
                                                  {idr, iot, Price{1, 2}, 500});
                    });
                }
            });
        }
    ...
    SECTION("modify offer assets with liabilities")
    {
        auto getLiabilities = [&](TestAccount& acc) {
            LedgerTxn ltx(app->getLedgerTxnRoot());
            auto account = iotchain::loadAccount(ltx, acc.getPublicKey());
            Liabilities res;
            if (account)
            {
        ...
        auto getAssetLiabilities = [&](TestAccount& acc, Asset const& asset) {
            LedgerTxn ltx(app->getLedgerTxnRoot());
            auto trust = iotchain::loadTrustLine(ltx, acc.getPublicKey(), asset);
            Liabilities res;
            if (trust)
            {
        ...
        for_versions_from(10, *app, [&] {
            ...
            {
                checkModifyAssets(iot, usd, usd, iot);
            }
            SECTION("buying native swap assets")
            {
                checkModifyAssets(usd, iot, iot, usd);
            }
            ...
            {
                checkModifyAssets(iot, usd, cur1, usd);
            }
            SECTION("selling native change buying asset")
            {
                checkModifyAssets(iot, usd, iot, cur1);
            }
            SECTION("selling native change both assets")
            {
                checkModifyAssets(iot, usd, cur1, cur2);
            }

            SECTION("buying native change buying asset")
            {
                checkModifyAssets(usd, iot, usd, cur1);
            }
            SECTION("buying native change selling asset")
            {
                checkModifyAssets(usd, iot, cur1, iot);
            }
            SECTION("buying native change both assets")
            {
                checkModifyAssets(usd, iot, cur1, cur2);
            }
            ...
            {
                checkModifyAssets(idr, usd, iot, usd);
            }
            SECTION("non-native change buying asset native")
            {
                checkModifyAssets(idr, usd, idr, iot);
            }
        });
    }
    ...
    SECTION("reserve and liabilities checks")
    {
        SECTION("when creating an offer")
        {
            SECTION("selling native")
            {
                ...
                for_versions_from(10, *app, [&] {
                    REQUIRE_THROWS_AS(
                        market.addOffer(acc1, {iot, idr, Price{2, 1}, 501}),
                        ex_MANAGE_SELL_OFFER_LOW_RESERVE);
                    root.pay(acc1, reserve + txfee);
                    REQUIRE_THROWS_AS(
                        market.addOffer(acc1, {iot, idr, Price{2, 1}, 501}),
                    ...
                    root.pay(acc1, txfee);
                    REQUIRE_THROWS_AS(
                        market.addOffer(acc1, {iot, idr, Price{2, 1}, 500}),
                        ex_MANAGE_SELL_OFFER_UNDERFUNDED);
                    root.pay(acc1, txfee);
                    market.requireChangesWithOffer({}, [&] {
                        return market.addOffer(acc1,
                                               {iot, idr, Price{2, 1}, 499});
                    });
                });
            }
            ...
            SECTION("buying native")
            {
                ...
                for_versions_from(10, *app, [&] {
                    ...
                    REQUIRE_THROWS_AS(
                        market.addOffer(acc1, {idr, iot, Price{2, 1}, 501}),
                        ex_MANAGE_SELL_OFFER_LOW_RESERVE);
                    root.pay(acc1, reserve + txfee);
                    REQUIRE_THROWS_AS(
                        market.addOffer(acc1, {idr, iot, Price{2, 1}, 501}),
                    ...
                    root.pay(acc1, txfee);
                    REQUIRE_THROWS_AS(
                        market.addOffer(acc1, {idr, iot, Price{2, 1}, 500}),
                        ex_MANAGE_SELL_OFFER_UNDERFUNDED);
                    root.pay(acc1, txfee);
                    market.requireChangesWithOffer({}, [&] {
                        return market.addOffer(acc1,
                                               {idr, iot, Price{2, 1}, 499});
                    });
                });
            }
        ...
        SECTION("when modifying an offer")
        {
            SECTION("selling native")
            {
                ...
                for_versions_from(10, *app, [&] {
                    auto offer = market.requireChangesWithOffer({}, [&] {
                        return market.addOffer(acc1,
                                               {iot, idr, Price{2, 1}, 250});
                    });
                    auto offerID = offer.key.offerID;

                    REQUIRE_THROWS_AS(
                        market.updateOffer(acc1, offerID,
                                           {iot, idr, Price{2, 1}, 501}),
                        ex_MANAGE_SELL_OFFER_LINE_FULL);
                    root.pay(acc1, txfee);
                    REQUIRE_THROWS_AS(
                        market.updateOffer(acc1, offerID,
                                           {iot, idr, Price{2, 1}, 500}),
                        ex_MANAGE_SELL_OFFER_UNDERFUNDED);
                    root.pay(acc1, txfee);
                    market.requireChangesWithOffer({}, [&] {
            ...
            SECTION("buying native")
            {
                ...
                for_versions_from(10, *app, [&] {
                    ...
                    auto offer = market.requireChangesWithOffer({}, [&] {
                        return market.addOffer(acc1,
                                               {idr, iot, Price{2, 1}, 250});
                    });
                    auto offerID = offer.key.offerID;

                    REQUIRE_THROWS_AS(
                        market.updateOffer(acc1, offerID,
                                           {idr, iot, Price{2, 1}, 501}),
                        ex_MANAGE_SELL_OFFER_LINE_FULL);
                    root.pay(acc1, txfee);
                    REQUIRE_THROWS_AS(
                        market.updateOffer(acc1, offerID,
...
TEST_CASE("liabilities match created offer", "[tx][offers]")
{
    ...
    auto checkLiabilities = [&](int64_t sellingBalance, int64_t buyingLimit,
        ...
        int64_t offerAmount = 0;
        {
            ...
            auto entry =
                iotchain::loadOffer(ltx, a1.getPublicKey(), offer.key.offerID);
            liabilities =
