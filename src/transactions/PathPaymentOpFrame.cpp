...
PathPaymentOpFrame::doApply(AbstractLedgerTxn& ltx)
{
    ...
    if (ltx.loadHeader().current().ledgerVersion < 8)
    {
        doesSourceAccountExist =
            (bool)iotchain::loadAccountWithoutRecord(ltx, getSourceID());
    }

    if (!bypassIssuerCheck)
    {
        if (!iotchain::loadAccountWithoutRecord(ltx, mPathPayment.destination))
        {
    ...
    {
        auto destination = iotchain::loadAccount(ltx, mPathPayment.destination);
    ...
    else
    {
        ...
        {
            auto issuer =
                iotchain::loadAccountWithoutRecord(ltx, getIssuer(curB));
            if (!issuer)
            {
        ...
        auto destLine =
            iotchain::loadTrustLine(ltx, mPathPayment.destination, curB);
        if (!destLine)
        {
    ...
    for (int i = (int)fullPath.size() - 1; i >= 0; i--)
    {
        ...
        {
            if (!iotchain::loadAccountWithoutRecord(ltx, getIssuer(curA)))
            {
                innerResult().code(PATH_PAYMENT_NO_ISSUER);
    ...
    if (curB.type() == ASSET_TYPE_NATIVE)
    {
        ...
        {
            sourceAccount = iotchain::loadAccount(ltx, getSourceID());
            if (!sourceAccount)
            {
                innerResult().code(PATH_PAYMENT_MALFORMED);
                return false;
