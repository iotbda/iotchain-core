 ...
 * `IOTChainMessage`, in the file src/xdr/IOTChain-overlay.x
 *
 ...
 *
 * The `IOTChainMessage` union contains 3 logically distinct kinds of message:
 *
 ...
 *    TRANSACTION and ICP_MESSAGE
 *
 *  - Two-way anycast messages requesting a value (by hash) or providing it:
 *    GET_TX_SET, TX_SET, GET_ICP_QUORUMSET, ICP_QUORUMSET, GET_ICP_STATE
 *
...
class OverlayManager
{
  public:
    ...
    // Herder.
    virtual void broadcastMessage(IOTChainMessage const& msg,
                                  bool force = false) = 0;
    ...
    virtual void recvFloodedMsg(IOTChainMessage const& msg,
                                Peer::pointer peer) = 0;
    ...
    virtual void
    recordDuplicateMessageMetric(IOTChainMessage const& iotchainMsg) = 0;
