...
void
OverlayManagerImpl::recvFloodedMsg(IOTChainMessage const& msg,
                                   Peer::pointer peer)
{
...
void
OverlayManagerImpl::broadcastMessage(IOTChainMessage const& msg, bool force)
{
...
OverlayManagerImpl::recordDuplicateMessageMetric(
    IOTChainMessage const& iotchainMsg)
{
    bool flood = false;
    if (iotchainMsg.type() == TRANSACTION || iotchainMsg.type() == ICP_MESSAGE)
    {
        flood = true;
    }
    else if (iotchainMsg.type() != TX_SET && iotchainMsg.type() != ICP_QUORUMSET)
    {
        return;
    }
    ...
    size_t size = xdr::xdr_argpack_size(iotchainMsg);
    auto opaque = xdr::xdr_to_opaque(iotchainMsg);
