...
class HerderStub : public HerderImpl
{
  ...
  private:
    EnvelopeStatus
    recvICPEnvelope(ICPEnvelope const& envelope) override
    {
...
makeEnvelope(int id)
{
    ...
    auto result = ICPEnvelope{};
    result.statement.slotIndex = ++slotIndex;
    result.statement.pledges.type(ICP_ST_CONFIRM);
...
TEST_CASE("ItemFetcher fetches", "[overlay][ItemFetcher]")
{
    ...
    auto checkFetchingFor = [&itemFetcher](Hash hash,
                                           std::vector<ICPEnvelope> envelopes) {
