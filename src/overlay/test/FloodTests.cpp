...
TEST_CASE("Flooding", "[flood][overlay][acceptance]")
{
    ...
    auto test = [&](std::function<void(int)> inject,
        ...
        // per tx so that we can easily identify them)
        {
            LedgerEntry gen;
            {
                ...
                gen = iotchain::loadAccount(ltx, root.getPublicKey()).current();
            }
    ...
    SECTION("transaction flooding")
    {
        auto injectTransaction = [&](int i) {
            ...
            auto msg = tx1->toIOTChainMessage();
    ...
    SECTION("icp messages flooding")
    {
        // ICP messages depend on
        // a quorum set
        ...
        auto injectICP = [&](int i) {
            ...
            // build an ICP message for the next ledger
            auto ct = std::max<uint64>(
                lcl.header.icpValue.closeTime + 1,
                VirtualClock::to_time_t(inApp->getClock().now()));
            IOTChainValue sv(txSet.getContentsHash(), ct, emptyUpgradeSteps,
            ...
            envelope.signature = keys[i].sign(xdr::xdr_to_opaque(
                inApp->getNetworkID(), ENVELOPE_TYPE_ICP, st));

            // inject the message
            REQUIRE(herder.recvICPEnvelope(envelope, qset, txSet) ==
                    Herder::ENVELOPE_STATUS_READY);
        };

        auto ackedICP = [&](std::shared_ptr<Application> app) {
            // checks if an app received and processed all ICP messages
            size_t okCount = 0;
            auto const& lcl =
            ...
            auto state =
                herder.getICP().getCurrentState(lcl.header.ledgerSeq + 1);
        ...
        auto quorumAdjuster = [&](ICPQuorumSet const& qSet) {
            auto resQSet = qSet;
            ICPQuorumSet sub;
