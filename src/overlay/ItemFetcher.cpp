...
void
ItemFetcher::fetch(Hash itemHash, const ICPEnvelope& envelope)
{
...
void
ItemFetcher::stopFetch(Hash itemHash, const ICPEnvelope& envelope)
{
...
{
    auto result = std::vector<ICPEnvelope>{};
    auto iter = mTrackers.find(itemHash);
    if (iter == mTrackers.end())
    {
        return result;
    ...
    std::transform(
        ...
        [](std::pair<Hash, ICPEnvelope> const& x) { return x.second; });
    return result;
}
...
ItemFetcher::recv(Hash itemHash)
{
    ...
    if (iter != mTrackers.end())
    {
        // this code can safely be called even if recvICPEnvelope ends up
        ...
        while (!tracker->empty())
        {
            mApp.getHerder().recvICPEnvelope(tracker->pop());
        }
