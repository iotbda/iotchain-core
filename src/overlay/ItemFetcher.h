...
using ICPQuorumSetPtr = std::shared_ptr<ICPQuorumSet>;
...
class ItemFetcher : private NonMovableOrCopyable
{
  public:
     ...
     */
    void fetch(Hash itemHash, const ICPEnvelope& envelope);
     ...
     */
    void stopFetch(Hash itemHash, const ICPEnvelope& envelope);
     ...
     */
    std::vector<ICPEnvelope> fetchingFor(Hash itemHash) const;
