...
class Tracker
{
  private:
    ...
    VirtualTimer mTimer;
    std::vector<std::pair<Hash, ICPEnvelope>> mWaitingEnvelopes;
    Hash mItemHash;
  ...
  public:
     ...
     */
    const std::vector<std::pair<Hash, ICPEnvelope>>&
     ...
     * is received.
     */
    void listen(const ICPEnvelope& env);

    /**
     * Stops tracking envelope @p env.
     */
    void discard(const ICPEnvelope& env);
