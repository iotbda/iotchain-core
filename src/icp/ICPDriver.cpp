...
std::string
ICPDriver::getValueString(Value const& v) const
{
...
std::string
ICPDriver::toStrKey(PublicKey const& pk, bool fullKey) const
{
    return fullKey ? KeyUtils::toStrKey(pk) : toShortString(pk);
}

std::string
ICPDriver::toShortString(PublicKey const& pk) const
{
...
uint64
ICPDriver::computeHashNode(uint64 slotIndex, Value const& prev, bool isPriority,
...
uint64
ICPDriver::computeValueHash(uint64 slotIndex, Value const& prev,
...
ICPDriver::computeTimeout(uint32 roundNumber)
{
