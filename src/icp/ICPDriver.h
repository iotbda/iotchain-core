...
class ICPDriver
{
  public:
    ...
    // Envelope signature
    virtual void signEnvelope(ICPEnvelope& envelope) = 0;

    // Retrieves a quorum set from its hash
    //
    // All ICP statement (see `ICPNomination` and `ICPStatement`) include
    // a quorum set hash.
    // ICP does not define how quorum sets are exchanged between nodes,
    // hence their retrieval is delegated to the user of ICP.
    // The return value is not cached by ICP, as quorum sets are transient.
    //
    // `nullptr` is a valid return value which cause the statement to be
    // considered invalid.
    virtual ICPQuorumSetPtr getQSet(Hash const& qSetHash) = 0;

    // Users of the ICP library should inherit from ICPDriver and implement the
    // virtual methods which are called by the ICP implementation to
    // abstract the transport layer used from the implementation of the ICP
    // protocol.

    // Delegates the emission of an ICPEnvelope to the user of ICP. Envelopes
    // should be flooded to the network.
    virtual void emitEnvelope(ICPEnvelope const& envelope) = 0;
    ...
    // the following methods are used for monitoring of the ICP subsystem
    ...
    virtual void
    startedBallotProtocol(uint64 slotIndex, ICPBallot const& ballot)
    {
    }

    // `acceptedBallotPrepared` every time a ballot is accepted as prepared
    virtual void
    acceptedBallotPrepared(uint64 slotIndex, ICPBallot const& ballot)
    {
    }

    // `confirmedBallotPrepared` every time a ballot is confirmed prepared
    virtual void
    confirmedBallotPrepared(uint64 slotIndex, ICPBallot const& ballot)
    {
    }

    // `acceptedCommit` every time a ballot is accepted commit
    virtual void
    acceptedCommit(uint64 slotIndex, ICPBallot const& ballot)
    {
    }
    ...
    // the local node.
    virtual void
    ballotDidHearFromQuorum(uint64 slotIndex, ICPBallot const& ballot)
    {
    }
};
}
