...
TrustLineWrapper::NonIssuerImpl::addBalance(LedgerTxnHeader const& header,
                                            int64_t delta)
{
    return iotchain::addBalance(header, mEntry, delta);
}
...
{
    return iotchain::getBuyingLiabilities(header, mEntry);
}
...
TrustLineWrapper::NonIssuerImpl::getSellingLiabilities(
    LedgerTxnHeader const& header)
{
    return iotchain::getSellingLiabilities(header, mEntry);
}
...
{
    return iotchain::addBuyingLiabilities(header, mEntry, delta);
}
...
TrustLineWrapper::NonIssuerImpl::addSellingLiabilities(
    LedgerTxnHeader const& header, int64_t delta)
{
    return iotchain::addSellingLiabilities(header, mEntry, delta);
}

bool
TrustLineWrapper::NonIssuerImpl::isAuthorized() const
{
    return iotchain::isAuthorized(mEntry);
}
...
{
    return iotchain::getAvailableBalance(header, mEntry);
}
...
TrustLineWrapper::NonIssuerImpl::getMaxAmountReceive(
    LedgerTxnHeader const& header) const
{
    return iotchain::getMaxAmountReceive(header, mEntry);
}
